...
int ClosestWaypoint(double x, double y, const std::vector<double>& maps_x, const std::vector<double>& maps_y)
{
	...
	for(int i = 0; i < maps_x.size(); i++)
	{
		...
		double dist = cardial_distance(x,y,map_x,map_y);
		if(dist < closestLen)
		{
			closestLen = dist;
			closestWaypoint = i;
...
std::vector<double> getFrenet(double x, double y, double theta, const std::vector<double>& maps_x, const std::vector<double>& maps_y)
{
	...
	double frenet_d = cardial_distance(x_x,x_y,proj_x,proj_y);

	//see if d value is positive or negative by comparing it to a center point

	double center_x = 1000-maps_x[prev_wp];
	double center_y = 2000-maps_y[prev_wp];
	double centerToPos = cardial_distance(center_x,center_y,x_x,x_y);
	double centerToRef = cardial_distance(center_x,center_y,proj_x,proj_y);
	...
	{
		frenet_s += cardial_distance(maps_x[i],maps_y[i],maps_x[i+1],maps_y[i+1]);
	}

	frenet_s += cardial_distance(0,0,proj_x,proj_y);
